import { ApplicationCommandOptionType, ChatInputCommandInteraction, Client, Colors, CommandInteraction } from "discord.js";
...
export async function run(
    ...
    for (const dbMember of members) {
        ...
        const member = await interaction.guild?.members
            .fetch(dbMember.discordID).catch(() => log(`[lead] Failed to fetch ${dbMember.username}`));
        if (!member) continue;
    ...
    await interaction.editReply({
        content: "Voici le classement des contributeurs :" + staff ? "\n*Les membres du staff ne sont pas affichées.*" : "",
